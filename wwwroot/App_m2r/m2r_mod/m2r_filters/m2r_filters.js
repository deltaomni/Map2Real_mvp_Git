 ...
 * This file set dfinitions for filters
...
function m2r_filters_start() {

	m2r_filters_set_operator()

	document.getElementById("m2r_filter_form").addEventListener('change', m2r_trips_filters_get_change);
	document.getElementById("m2r_uix_date_range").addEventListener('change', m2r_trips_filters_get_change);


	// buttons options Atualizar/Limpar at the top of the form
...
function m2r_trips_filters_get_change(e) {
	...
	//console.log(e, id, checked, value, name);
	...
	if (data == 'm2r_range_main') {
		m2r_trips_clear_filters("date_range", null); // clear selected options for date range
		_("option-3-10").value = _('m2r_date_from').value;
		_("option-3-11").value = _('m2r_date_to').value;
	...
	if (data == 'date_range') {
		...
		m2r_trips_set_date_filters(null);  // set time range.
	}
...
function m2r_trips_set_date_filters(value) {
	...
	var d_start = _("option-3-10").value;
	if (d_start) {
		//console.log(d_start);
		var custom_start = new Date(d_start)
		//console.log(d_start, custom_start);
		custom_start = parseInt((custom_start.getTime() / 1000).toFixed(0));
	}

	var d_finish = _("option-3-11").value;
	if (d_finish) {
		var custom_finish = new Date(d_finish);
		custom_finish = parseInt((custom_finish.getTime() / 1000).toFixed(0));
	}
	...
	switch (value) {
		case 'today':
			...
			date_start[0].el.value = today_unix - _UTC;
			date_finish[0].el.value = tomorow_unix - _UTC;
			break;
		...
		case 'yesterday':
			...
			date_start[0].el.value = yesterday_unix - _UTC;
			date_finish[0].el.value = today_unix - _UTC;
		...
		case 'last7dd':
			...
			date_start[0].el.value = last7dd_unix - _UTC;
			date_finish[0].el.value = tomorow_unix - _UTC;
		...
		case 'thisMo':
			...
			date_start[0].el.value = thisMoDate_unix - _UTC;
			date_finish[0].el.value = tomorow_unix - _UTC;
		...
		case 'last30dd':
			...
			date_start[0].el.value = last30dd_unix - _UTC;
			date_finish[0].el.value = tomorow_unix - _UTC;
		...
		default:
			if (custom_start && custom_finish) {
				date_start[0].el.value = custom_start - _UTC;
				date_finish[0].el.value = custom_finish - _UTC;
			}
			break;
	}
}
